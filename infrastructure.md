...
| 9092 | kapacitord | **no — bound to localhost** (2026-06-02) | local CLI only |
...
- **Staging (home-dev) is now the fully-hardened reference (2026-06-02):** grafana bound `127.0.0.1:3000` (set `http_addr` in grafana.ini, backup `.bak.20260602`) — reachable only via nginx 443; influxd bound `127.0.0.1:8086` — reachable only via SSH tunnel; mosquitto localhost `1883` + the mTLS `:8883` prototype (open, cert-gated). External `:3000`/`:8086` refused; `:80→443` redirect now in place (matches prod). **Consistency scan 2026-06-02 (both hosts, from pidevserver):** identical posture except the intended differences — prod's `3000`/`8080` still open pending the 443 sign-off, `8883` open on home-dev (prototype) vs staged/deferred on prod, and no kapacitor on home-dev.
- **Prod — gate on user sign-off** that the 443 paths (grafana `/`, upload `/upload/`) work before closing direct ports. nginx is live on prod with direct 3000/8080 still open in parallel for exactly this transition. **Status 2026-06-02: users chased for sign-off; binding 3000/8080 is queued pending their confirmation.** When confirmed: grafana `http_addr = 127.0.0.1` in `grafana.ini` (restart grafana); hamon-upload bind `:8080` → `127.0.0.1` (restart service); then re-scan from pidevserver to confirm `3000`/`8080` closed and `/` + `/upload/` still served via 443. End-state then = **22 + 443** on prod (+ `8883` on home-dev).
- **Bind prod to localhost:** ✅ **influxd** (`8086`), **mosquitto** (`1883`), and **kapacitor** (`9092`, `[http] bind-address = "127.0.0.1:9092"`, backup `kapacitor.conf.bak.<ts>`; deadman task unaffected) all done. **Remaining (gated on the 443 sign-off above):** grafana → `127.0.0.1:3000` (https upstream already proxied), hamon-upload → `127.0.0.1:8080` (already proxied via https upstream — no http/keep-auth rework needed).
  - **External scan from pidevserver 2026-06-02 confirms:** OPEN = 22, 80, 443, 3000, 8080; closed = 1883, 8086, 8883, 9092.
