...
- **mosquitto (1883) on prod — must stay reachable, secure via TLS+auth (NOT close).** It is the ingest feed: remote Pi-based site servers *publish* into this broker over the internet; the local `mqread.js` subscribes and writes to influx (`src/mqwrite.js`/`mqread.js` both `mqtt://localhost`). Currently mosquitto 1.6.9 with **no auth (anonymous)** — internet scanners are already connecting (45.79.207.181, 209.38.248.17, Censys, etc.). Lock down with **TLS + client-cert (mutual TLS) auth**, `allow_anonymous false`; keep a separate `127.0.0.1` plaintext listener for the local writer; migrate the remote Pis to the TLS listener; then retire plaintext public access. **Decided approach = Option A (deferred, pick up later):** mosquitto terminates its own TLS on **`:8883`** with client-cert auth + per-site ACLs (`use_identity_as_username`) — **not** behind nginx (nginx is HTTP-only; its `stream` module could TCP-proxy MQTT but would hide the client identity and break per-site ACLs). nginx stays web-only. End-state public ports: **22 (ssh) + 443 (nginx) + 8883 (mosquitto TLS)**.
  ...
  - **Remaining to productionise:** define real per-site CNs (= location names used in `knx/<site>/…`), issue+ship each remote Pi its cert/key + the CA, migrate the Pis from plaintext to the `:8883` TLS listener, then on prod retire public plaintext `1883` (keep the localhost plaintext for the writer). Prod also needs mosquitto ≥2.0 for `use_identity_as_username`/per-listener ACLs (2.6 upgrade path TBD). Provider firewall must allow `8883`. On home-dev `8883` is already internet-exposed but cert-gated, so safe.
